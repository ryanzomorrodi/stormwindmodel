...
```{r echo = FALSE}
var_names <- tibble::tibble(var = c("`vmax`",
                                       "`vmax_sfc_sym`",
                                       "`vmax_gl`",
                                       "`tclat`",
                                       "`tclon`",
...
floyd_states <- map("state", plot = FALSE, fill = TRUE) %>%
  st_as_sf() %>% 
  filter(ID %in% c("north carolina", "south carolina", "maryland",
...
```{r eval = FALSE}
  ...
  if(diff(range(hurr_track$tclon)) > 300){
    hurr_track <- hurr_track %>%
      mutate(tclon = ifelse(tclon > 0, tclon, tclon + 360))
  }
```
...
```{r eval = FALSE}
    ...
    mutate(tclon = ((tclon + 180) %% 360) - 180)
```
